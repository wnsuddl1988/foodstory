...
4. **이미지:** 본문 중간에 **2~3개** 삽입
   - 형식: `![설명](이미지URL)`
   - **반드시 글의 주제·내용과 직접 연관된 이미지만 사용** (관련 없는 일반 사진 금지)
   - 이미지는 글의 문맥 흐름에 맞는 위치에 배치 — 해당 내용을 설명하거나 보완하는 시점에 삽입
   - 아래 3개 사이트 중 검색 결과 URL을 바로 사용 (검색 1회 원칙, 과도한 시간/토큰 낭비 금지)
     - https://unsplash.com/ko
     - https://www.pexels.com/ko-kr/
     - https://pixabay.com/ko/
   - 검색 1회로 URL을 찾지 못하면 즉시 해당 이미지를 건너뛰고 글 작성을 계속 진행한다
   - `heroImage`는 반드시 외부 URL 문자열로 지정 (Pexels/Unsplash 등 — 로컬 파일 사용 불가)
